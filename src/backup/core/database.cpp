...
void Database::initial(QString name)
{
    ...
    database.setDatabaseName("/var/lib/harbour-dolphin/database/" + name + ".sqlite");
    ...
    query.exec("PRAGMA default_cache_size =32768");
...
QVector<QString> Database::load(QString sql)
{
...
{
    QString sql = "UPDATE " + name + " SET frequency = frequency + 1 WHERE word = \"" + word + "\"";
    QSqlQuery query(database);
    query.exec(sql);
}
...
QStringList Database::predict(int mode, QString word, QString size)
{
    QString sql;
    switch(mode) {
    case 0:
        sql = "SELECT phrase FROM predict WHERE word = \"" + word + "\" ORDER BY frequency DESC, sorting DESC LIMIT 0, " + size;
        return Database::load(sql);
        break;
    case 1:
        int prefix = word.length() + 1;
        sql = "SELECT SUBSTR(phrase," + QString::number(prefix) + ") FROM predict WHERE phrase GLOB \"" + word + "*\" AND LENGTH(phrase) - " + QString::number(prefix) + " > 0 ORDER BY frequency DESC LIMIT 0, " + size;
        break;
    }
...
QStringList Database::swype(QString keys, QString count, QString size)
{
    ...
    sql = "SELECT word FROM swype WHERE word GLOB \"" + keys + "\" AND LENGTH(word) > " + count + " ORDER BY LENGTH(word) ASC LIMIT 0, " + size;
